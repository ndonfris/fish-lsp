...
export namespace Range {
    ...
    export function fromArray([start, end]: [PositionArray, PositionArray]) : Range {
      const startPos = Position.fromArray(start);
      const endPos = Position.fromArray(end);
...
export namespace FoldingRange {
    ...
    export function fromLspFoldingRange(range: LSP.FoldingRange) : FoldingRange {
      const { start, end } = Range.fromArray([[range.startLine, range.startCharacter || -1], [range.endLine, range.endCharacter || -1]]);
      return create(
        start,
        end,
        range.kind || '',
