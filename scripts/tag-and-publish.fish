...
if set -q _flag_build_release_assets
    ...
    else if test -f man/man1/fish-lsp.1
        # Support legacy path as fallback
        cp man/man1/fish-lsp.1 release-assets/
        if test $status -eq 0
            echo "✅ Copied man/man1/fish-lsp.1 to release-assets/ (legacy path)"
        else
            echo $RED"❌ Failed to copy man page from legacy path"
            exit 1
        end
    else
        echo "⚠️  Warning: Man page not found at man/fish-lsp.1 or man/man1/fish-lsp.1"
    end
