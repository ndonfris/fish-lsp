...
export function applyFormatterSettings(root: SyntaxNode, options: FishFormattingOptions) {
  ...
  if (!options.insertSpaces) result = result.replace(new RegExp(tabStr, 'g'), '\t');
  if (options.trimTrailingWhitespace) result = result.split('\n').map(line => line.trimEnd()).join('\n');
  if (options.trimFinalNewlines) result = result.trimEnd();
  if (options.insertFinalNewline) result = result.trimEnd() + '\r\n';
...
export function applyFormattedTextInRange(formattedText: string, range: Range): string {
  ...
  if (start.line === end.line) return newText[start.line]!.slice(start.character, end.character);
