...
/**
 * Computes scope spans for a variable within a single document.
 *
 * The document is partitioned into contiguous `ScopeSpan` segments:
 *  - `'include'` spans where nodes matching the variable are valid references
 ...
 *
 * @param variableSymbols Pre-filtered same-name variable symbols from the target document
 * @returns Sorted, non-overlapping array of ScopeSpans covering the document
 */
export function buildScopeSpans(
  rootNode: SyntaxNode,
  variableSymbols: FishSymbol[],
): ScopeSpan[] {
  if (variableSymbols.length === 0) {
    return [{ startIndex: rootNode.startIndex, endIndex: rootNode.endIndex, tag: 'include' }];
  }

  const localSymbols = variableSymbols.filter(s => s.isLocal());
  const globalSymbols = variableSymbols.filter(s => s.isGlobal());
  ...
  for (const ls of localSymbols) {
    ...
    const hasBroaderDef = variableSymbols.some(other =>
      !other.equals(ls)
