 ...
 */
//export function getArray(node: SyntaxNode): SyntaxNode[] {
  //const queue: SyntaxNode[] = [node]
//
  //while (queue.length) {
    //const n = queue.shift()
 ...
 * @returns {SyntaxNode[]} - all nodes seen in the document.
 */
export function getChildrenArray(node: SyntaxNode): SyntaxNode[] {
...
// 
//   if (!filename.endsWith('.fish') && !filename.endsWith('.gawk')) {
