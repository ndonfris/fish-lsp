...
const Pipes: FishCompletionItem[] = [
  ...
  {
    label: '>&2',
    detail: 'STDOUT_TO_STDERR >&2',
    insertText: '>&2',
    documentation: 'To redirect standard output to standard error, use >&2',
  },
  {
    label: '2>&1',
    detail: 'STDERR_TO_STDOUT 2>&1',
    insertText: '2>&1',
    documentation: 'To redirect standard error to standard output, use 2>&1',
  },
] as FishCompletionItem[];
...
const statusNumbersRaw = [
  ...
  {
    label: '128',
    detail: 'Status Invalid Exit through Signal',
    documentation: 'means that a process exited through a signal, the exit status will be 128 plus the number of the signal',
    examples: [
      CompletionExample.create('Exit status 141 signifies cat was killed by signal 13 (128 + 13 = 141) `SIGPIPE`',
        '## NOTE: It’s possible for the first command to fail while the second succeeds.',
        '##       One common example is when the second program quits early.',
        '##       In example below, `head -n 50` can succeed before `cat file1 file2` is finished',
        '##       resulting in failure exit status to fire closing `cat`',
        '',
        ...
        '# 141 0',
        '',
        '## NOTE: You can also use the `$fish_kill_signal` to see the signal number',
      ),
    ],
  },
];
