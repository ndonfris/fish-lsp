...
import lsp, {Connection} from 'vscode-languageserver';
//import { LspClient } from '../fish-lsp-client';
...
export class LspClientLogger implements Logger {
    private loggerConnection;
    constructor(protected context: Context, protected level: lsp.MessageType) { 
        //let childProcess = cp.spawn(context.connection);
         this.loggerConnection = rpc.createMessageConnection(
    ...
    protected sendMessage(severity: lsp.MessageType, messageObjects: any[], options?: { overrideLevel?: boolean; }): void {
        if (this.level >= severity || options?.overrideLevel) {
            ...
            this.loggerConnection.sendNotification(notify)
            this.loggerConnection.listen()
        }
    }
