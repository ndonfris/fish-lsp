...
export namespace Node {
    ...
    export function debugSyntaxNode(node: SyntaxNode) : string {
        const shortLog = (node: SyntaxNode) => {
            return JSON.stringify({text: node.text, type: node.type}, null, 2)
        }
        let result = {
            id: node.id,
            text: node.text,
            ...
            children: node.children.map(shortLog).join('\n'),
            startIndex: node.startIndex,
            endIndex: node.endIndex,
        }

        let previousSibling = node.previousSibling ? shortLog(node.previousSibling) : null;
        let previousNamedSibling = node.previousNamedSibling ? shortLog(node.previousNamedSibling) : null;
        let nextSibling = node.nextSibling ? shortLog(node.nextSibling) : null;
        let nextNamedSibling = node.nextNamedSibling ? shortLog(node.nextNamedSibling) : null;
        let firstChild = node.firstChild ? shortLog(node.firstChild) : null;
        let lastChild = node.lastChild ? shortLog(node.lastChild) : null;
