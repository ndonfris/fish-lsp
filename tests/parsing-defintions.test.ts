  ...
  describe('test SyntaxNode checking', () => {
    ...
    describe('variable names', () => {
      ...
      it('set -q with excludeQuery=false', () => {
        const source = 'set -ql foo (echo \'universal var\')';
        const { rootNode } = parser.parse(source);
        const foundNode = getChildNodes(rootNode).find((n) => Parsers.set.isSetVariableDefinitionName(n, false));
        expect(foundNode).toBeDefined();
        expect(foundNode?.text).toBe('foo');
      });
      it('set -e index target with excludeQuery=false', () => {
        const source = 'set -e PATH[1]';
        const { rootNode } = parser.parse(source);
        const foundNode = getChildNodes(rootNode).find((n) => Parsers.set.isSetVariableDefinitionName(n, false));
        expect(foundNode).toBeDefined();
        expect(foundNode?.text).toBe('PATH');
      });
      it('read', () => {
        const source = 'read -l foo bar baz';
        const { rootNode } = parser.parse(source);
